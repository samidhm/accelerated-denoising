...
def activate_compositing(path: str):
    ...
    normalize = bpy.context.scene.node_tree.nodes.new(type='CompositorNodeNormalize')
    nodes["depth"] = bpy.context.scene.node_tree.nodes.new('CompositorNodeOutputFile')
    nodes["depth"].base_path = path + "/depth/"

    nodes["glossy_color"] = bpy.context.scene.node_tree.nodes.new('CompositorNodeOutputFile')
    nodes["glossy_color"].base_path = path + "/glossy_color/"
    nodes["glossy_color"].format.color_mode = "RGB"

    nodes["diffuse_color"] = bpy.context.scene.node_tree.nodes.new('CompositorNodeOutputFile')
    nodes["diffuse_color"].base_path = path + "/diffuse_color/"
    nodes["diffuse_color"].format.color_mode = "RGB"

    nodes["normal"] = bpy.context.scene.node_tree.nodes.new('CompositorNodeOutputFile')
    ...
    bpy.context.scene.node_tree.links.new(normalize.outputs['Value'], nodes["depth"].inputs['Image'])
...
def render(
        ...
        output_folder: str, 
        samples: int = 1, 
        composite_output_nodes: dict[str, bpy.types.CompositorNodeOutputFile] = None
    ):
    ...
    bpy.context.scene.render.filepath = f"{output_folder}/samples_{samples}/{file_name}.png"

    bpy.ops.render.render(write_still=True)

    if composite_output_nodes is not None:
        for key in composite_output_nodes.keys():
            os.rename(f"{output_folder}/{key}/Image0001.png", f"{output_folder}/{key}/{file_name}.png")
        
        if not os.path.exists(f"{output_folder}/view_space_matrix"):
            os.makedirs(f"{output_folder}/view_space_matrix")

        np.save(f"{output_folder}/view_space_matrix/{file_name}.npy", np.array(camera.matrix_world.inverted().to_3x3()))
...
if __name__ == "__main__":
    ...
    for samples in sample_count:
        ...
        for (x, y), z, eu_x, eu_z in itertools.product(*axes):
            ...
            render(camera, Obj3D(x, y, z), Obj3D(eu_x, 0, eu_z), 
                args.output_folder, samples, nodes)
